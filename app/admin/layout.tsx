...
export default async function AdminLayout({
  ...
  // Always resolve admin via auth.getUser()
  // profiles.id = auth.uid()
  // Single auth gate for ALL /admin/* pages
  ...
  if (!diagnosis.ok && !uiProbe) {
    ...
    if (diagnosis.role) {
      params.set("role", diagnosis.role);
    }

    if (diagnosis.authUserId) {
      params.set("uid", diagnosis.authUserId);
    }

    redirect(`/login?${params.toString()}`);
  }

  const email = diagnosis.ok
