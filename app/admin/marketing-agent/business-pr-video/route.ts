...
import { generateBusinessPrVideoFromForm, collectBusinessPrImages } from "@/lib/marketing-agent/business-pr-video";
...
function fail(error: unknown, status = 400): NextResponse {
  ...
  if (error instanceof Error) {
    ...
    if (error.name === "TimeoutError" || error.name === "AbortError") {
      return errorJson(
        "動画生成がタイムアウトしました。ローカルの FFmpeg 処理を確認してください。",
        504,
...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.VERCEL) {
      return errorJson(
        "事業PR動画の複数画像レンダリングは、この版ではローカル（ffmpeg）でのみ実行します。Cloud Run 連携は次の段階です。",
        503,
        { stage: "cloud-run" },
      );
    }
    ...
    if (!rawScript) {
      return errorJson("先に事業PRの台本を生成してください。", 400, {
        stage: "vercel",
      });
    }
    ...
    const result = await generateBusinessPrVideoFromForm({
      script,
      images,
      companyName,
    });
