...
export default async function EnglishMakerCasesPage({ searchParams }: PageProps) {
  const user = await getSessionUser();
  if (!user) {
    redirect(`/en/login?next=${encodeURIComponent("/en/maker/cases")}`);
  }
  if (user.role !== "maker") {
    redirect("/en/cases");
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-12 md:py-16" lang="en">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-muted">
            Track your BrandBridge listings. You can register products anytime
            after company setup.
          </p>
        </div>
        <Button href="/en/maker/cases/new">Register a product</Button>
      </header>
      ...
      <div className="mb-8 flex flex-wrap gap-3">
        <Button href="/en/maker/dashboard" variant="outline">
          Maker Dashboard
        </Button>
      ...
      {myCases.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border bg-surface px-5 py-10 text-center">
          <p className="text-sm text-muted">
            No products yet. Company setup is complete - register a product when
            you are ready.
          </p>
