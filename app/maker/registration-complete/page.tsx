...
export const metadata: Metadata = {
  title: "登録完了 | BrandBridge",
  description: "商品登録が完了しました。",
};
...
export default async function MakerRegistrationCompletePage({
  ...
  const latest =
    (created ? cases?.find((c) => c.id === created) : undefined) ??
    cases?.[0] ??
    null;
          ...
          <div className="mt-4 space-y-4">
            {latest.product_image_url ? (
              ...
              <img
                ...
                alt={latest.product_name ?? latest.title}
            ...
            <dl className="space-y-3 text-sm">
              {[
                ["商品名", latest.product_name ?? latest.title],
                ...
                [
                  ...
                  `${salesFormatLabel(latest.sales_format as never)}${
                    latest.is_exclusive ? "・独占販売" : ""
                  }`,
                ],
                ["販売条件", latest.sales_terms || latest.offer],
                [
                  "審査ステータス",
                  reviewStatusLabels[
                    latest.review_status as keyof typeof reviewStatusLabels
