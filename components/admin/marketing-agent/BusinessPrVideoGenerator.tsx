  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted">
            夜景、街、オフィス、人物、ブランドイメージなどを自由に混ぜられます。
          </p>
          {images.length > 0 ? (
        ...
        <SubmitButton pendingLabel="台本を生成中…">日本語で台本を作成</SubmitButton>
      </form>
      ...
      <div className="space-y-3 rounded-lg border border-teal/40 bg-surface p-4">
        ...
        <p className="text-xs text-muted">
          日本語ナレーション付きの 1080×1920 縦動画を作ります。公開・SNS投稿はしません。商品選択は不要です。
        </p>
              ...
              : videoStatus === "failed"
                ...
                : script
                  ? images.length < 2
                    ? "待機 — 画像を2枚以上追加してください"
                    : "待機 — 内容を確認して動画を生成できます"
                  : "待機 — 先に日本語で台本を作成してください"}
        </p>
        {videoStatus === "generating" ? (
          <p className="text-xs text-muted">経過: {elapsedSeconds}秒</p>
        ) : null}
        {videoStatus === "failed" ? (
          <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-800">
            <p className="font-medium">
              失敗{videoHttpStatus ? ` (HTTP ${videoHttpStatus})` : ""}
            </p>
            <p className="mt-1 text-xs">段階: {describePrVideoStage(videoStage)}</p>
          ...
          <ol className="space-y-3">
            {script.scenes.map((scene) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-muted">場所 / 人物 / カメラ</p>
                <p className="mt-1 text-navy">
                  {scene.location} / {scene.character} / {scene.camera}
                </p>
              </li>
            ))}
          </ol>
