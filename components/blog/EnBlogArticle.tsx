...
export function EnBlogArticle({ article }: EnBlogArticleProps) {
  const siteUrl = getSiteUrl();
  const path = enBlogPath(article.slug);
  const pageUrl = `${siteUrl}${path}`;
  const related = article.relatedSlugs
    .map((slug) => {
      const item = getEnBlogArticle(slug);
      return item
        ? { href: enBlogPath(item.slug), title: item.title }
        : null;
    })
    .filter((item): item is { href: string; title: string } => item !== null);

  const serviceHrefs = new Set(article.existingLinks.map((link) => link.href));
      ...
      {faqs.length > 0 ? (
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: jsonLdString(faqPageJsonLd(faqs)),
          }}
        />
      ) : null}
      ...
      <article className="border-b border-border bg-surface">
        <div className="mx-auto max-w-3xl px-5 py-12 md:py-16">
          <p className="text-xs text-muted">
            <Link href={EN_BLOG_HUB.path} className="text-teal hover:underline">
              {EN_BLOG_HUB.label}
            </Link>
          ...
          {faqs.length > 0 ? (
            <section className="mt-12 border-t border-border pt-10">
              <h2 className="font-[family-name:var(--font-shippori)] text-2xl text-navy md:text-3xl">
                {article.faqHeading ?? "FAQ"}
              </h2>
              <dl className="mt-6 space-y-6">
                {faqs.map((item) => (
                  <div key={item.q}>
                    <dt className="font-[family-name:var(--font-shippori)] text-lg text-navy">
                      {item.q}
                    </dt>
          ...
          {article.existingLinks.length > 0 ? (
            <section className="mt-12 border-t border-border pt-10">
              <h2 className="font-[family-name:var(--font-shippori)] text-2xl text-navy md:text-3xl">
                Related pages
              </h2>
