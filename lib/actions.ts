...
export async function upsertNegotiationTermsAction(input: {
  ...
  if (input.status === "submitted") {
    ...
    // messages ????????????????
    // ???????????????????????
    const { data: existingNotice, error: noticeCheckError } = await supabase
      .from("messages")
      .select("id")
    ...
    if (noticeCheckError) {
      return { error: noticeCheckError.message };
    }
...
export async function confirmNegotiationTermsAction(input: {
  negotiationId: string;
}) {
  let user;
  try {
  ...
  if (user.id === caseRow?.maker_id) {
    update.maker_confirmed_at = new Date().toISOString();
  }

  if (user.id === negotiation.partner_id) {
  ...
  if (terms.maker_confirmed_at && terms.partner_confirmed_at) {
    await supabase
      .from("negotiation_terms")
      .update({
        status: "agreed",
