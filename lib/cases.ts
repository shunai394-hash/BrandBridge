...
function mapCase(row: CaseWithMaker): Case {
  ...
  return {
    ...
    salesTrackRecord: row.sales_track_record ?? null,
    marketAvailabilityJpUs: row.market_availability_jp_us ?? null,
    leadTime: row.lead_time ?? null,
    initialOrderTerms: row.initial_order_terms ?? null,
    trademarkStatus: row.trademark_status ?? null,
    exclusiveDealOption: row.exclusive_deal_option ?? null,
    ...
    supportLanguages: row.support_languages ?? null,
...
export async function createCase(
  ...
  try {
    ...
    const insertPayload = {
      ...
      sales_track_record: normalized.salesTrackRecord.trim() || null,
      market_availability_jp_us:
        normalized.marketAvailabilityJpUs.trim() || null,
      lead_time: normalized.leadTime.trim() || null,
      initial_order_terms: normalized.initialOrderTerms.trim() || null,
      trademark_status: normalized.trademarkStatus.trim() || null,
      exclusive_deal_option: normalized.exclusiveDealOption.trim() || null,
      ship_from: normalized.shipFrom.trim() || null,
      currencies: normalized.currencies.trim() || null,
      incoterms: normalized.incoterms.trim() || null,
      certifications: normalized.certifications.trim() || null,
      support_languages: normalized.supportLanguages.trim() || null,
