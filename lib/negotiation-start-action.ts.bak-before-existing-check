...
export async function startNegotiationAction(input: {
  ...
  if (topic.length > TOPIC_MAX) {
    return {
      ok: false,
      error: `莉ｶ蜷阪・${TOPIC_MAX}譁・ｭ嶺ｻ･蜀・↓縺励※縺上□縺輔＞`,
      logs,
    };
  }
  ...
  const body =
    ...
    (hasAttachment ? "繝輔ぃ繧､繝ｫ繧呈ｷｻ莉倥＠縺ｾ縺励◆" : "莠､貂峨ｒ逕ｳ縺苓ｾｼ縺ｿ縺ｾ縺励◆");
  ...
  if (appError && appError.code !== "23505") {
    ...
    return {
      ok: false,
      error: `applications INSERT 螟ｱ謨・ ${appError.message}`,
      logs,
    };
  }
  ...
  const negotiationPayload = {
    ...
    topic,
    message: body,
    // Open chat immediately 窶・no maker approval gate
  ...
  if (negoError || !nego) {
    ...
    return {
      ok: false,
      error: `negotiations INSERT 螟ｱ謨・ ${negoError?.message ?? "unknown"}`,
      logs,
    };
  }
  ...
  // ----- 2) messages INSERT 窶・always runs after successful negotiation insert -----
...
export async function startNegotiationDraftAction(input: {
  ...
  const body =
    (input.body ?? input.message)?.trim() || "莠､貂峨ｒ逕ｳ縺苓ｾｼ縺ｿ縺ｾ縺励◆";
  const supabase = await createClient();
  ...
  if (existingNegotiation) {
    log("existing negotiation found", {
      id: existingNegotiation.id,
    });

    return {
      ok: true,
      id: existingNegotiation.id as string,
      logs,
    };
  }
  ...
  if (error || !nego) {
    ...
    return {
      ok: false,
      error: `negotiations INSERT 螟ｱ謨・ ${error?.message ?? "unknown"}`,
      logs,
    };
  }
...
export async function completeNegotiationOpeningAction(input: {
  ...
  if (negoError || !nego) {
    return { ok: false, error: "莠､貂峨′隕九▽縺九ｊ縺ｾ縺帙ｓ", logs };
  }
  if (nego.partner_id !== partner.id) {
    return { ok: false, error: "讓ｩ髯舌′縺ゅｊ縺ｾ縺帙ｓ", logs };
  }
  ...
  if (msgError || !msg) {
    ...
    return {
      ok: false,
      error: `messages INSERT 螟ｱ謨・ ${msgError?.message ?? "unknown"}`,
      id: input.negotiationId,
      logs,
    };
  }
